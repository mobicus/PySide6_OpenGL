...
class RenderWidget(QOpenGLWidget):
    ...
    def paintGL(self):
        ...
        # Get the location of the trasform matrix
        transform = self.program.uniformLocation("transform")
        # setup the transform matrix
        transMat = QMatrix4x4()
        transMat.translate( 0.5, -0.5, 0.0 )
        transMat.rotate( self.angle, 0.0, 0.0, 1.0)
        ...
        self.program.setUniformValue(transform, transMat)
        ...
        # draw with first transform
        f.glDrawElements(GL.GL_TRIANGLES, 6, GL.GL_UNSIGNED_INT, VoidPtr(0))

        # draw again with another transformation
        transMat = QMatrix4x4()
        scale = np.sin(self.timeStep)
    ...
    def play(self):
        if self.timer is None:
            ...
            self.timer.timeout.connect(self.updateTimeStep)
